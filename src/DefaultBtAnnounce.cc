...
//
    ...
    /*struct sockaddr_in rawSocketDescription;
    
    int dummySocket_ = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    
    if(dummySocket_==-1){
      A2_LOG_NOTICE("ERROR AL CREAR EL SOCKET DE TTL");
      return;
    }
    
    rawSocketDescription.sin_family = AF_INET;
    inet_aton((elem.first).c_str(),&rawSocketDescription.sin_addr);
    //sin.sin_addr.s_addr = (struct in_addr*)elem.first; 
    rawSocketDescription.sin_port = elem.second; 

    if (connect (dummySocket_, (struct sockaddr *)&rawSocketDescription,sizeof (rawSocketDescription)) == -1) 
    { 
      A2_LOG_NOTICE("ERROR AL CONECTAR EL SOCKET DE TTL");
      return;
    } 
    A2_LOG_NOTICE("SE CONECTO COOOOOL TTL");  
    ...
    */
    ...
    socket_->establishConnection((elem.first).c_str(),elem.second,false);
    
    string msg = "Hoila";    
    socket_->writeData(msg);
    
    size_t len = (long unsigned int)20;
    unsigned char data[100];
    socket_->readData((void*)data,len);
    
    A2_LOG_NOTICE(fmt("Data Recibida: %s",data));
